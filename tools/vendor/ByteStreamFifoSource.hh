...
// C++ header
//
// Vendored from yi-hack-v5's rRTSPServer with one addition: the pending
// buffer (fPendingData/fPendingLen/fPendingOff) that createNew() uses to
// serve the decodable tail it kept while draining the fifo.
...
private:
    ...
    // Pending bytes kept from the createNew() drain (tail from the last
    // SPS+PPS+IDR chain); served before any further fifo reads so the
    // first client always joins at a decodable point.
