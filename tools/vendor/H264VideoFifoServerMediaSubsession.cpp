...
FramedSource* H264VideoFifoServerMediaSubsession::createNewStreamSource(unsigned /*clientSessionId*/, unsigned& estBitrate) {
    ...
    // playTimePerFrame MUST be 0: a nonzero value sets every delivery's
    // fDurationInMicroseconds, and the RTP sink paces each PACKET by
    // that duration - at 50000 us the big reads (up to ~150 KB, dozens
    // of packets) stall the stream for tens of seconds and clients see
    // a single frozen frame (measured live: 12 s gaps between the
    // server's reads, ffplay stuck on one frame). The RTP timestamps
    // come from the source's gettimeofday, not the pacing.
    ByteStreamFifoSource* fileSource = ByteStreamFifoSource::createNew(envir(), fFileName, 0, 0);
