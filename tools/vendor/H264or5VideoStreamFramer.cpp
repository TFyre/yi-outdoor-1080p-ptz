...
#include <sys/time.h>   /* gettimeofday - wall-clock presentation times
                         * (yi-hack-v5 port; see the parse() change) */
...
unsigned H264or5VideoStreamParser::parse() {
  try {
    ...
    if (thisNALUnitEndsAccessUnit) {
      ...
      // (yi-hack-v5 port) This server's video source is a LIVE fifo, so
      // stamp the wall clock instead of advancing a counter at fFrameRate:
      // the counter ran at 20 fps while the true source rate is ~19.5 fps,
      // so the video RTP clock drifted 2.5% slow against the audio
      // substream's real-time clock (A/V desync growing ~1.5 s per minute).
      // The audio side gets the same treatment via -DPRES_TIME_CLOCK=1,
      // so both RTP clocks share the wall-clock epoch and stay in sync.
