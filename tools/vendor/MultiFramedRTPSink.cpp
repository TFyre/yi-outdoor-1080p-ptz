...
::afterGettingFrame1(unsigned frameSize, unsigned numTruncatedBytes,
  ...
  if (numFrameBytesToUse == 0 && frameSize > 0) {
    ...
    //
    // Vendored change: for our LIVE fifo source the source IS the
    // clock - never pace packets. The original advanced fNextSendTime
    // by the frame's duration, with a 66667 us PER-NAL fallback when
    // the duration is 0 (our subsession sets playTimePerFrame=0, and
    // the framer's computed duration can also come out 0), which caps
    // delivery at 15 NALs/s against a ~19.5 fps source: the fifo fills,
    // the producer's whole-or-drop kicks in, and clients see a burst
    // followed by a frozen stream (the measured 6-8 s read gaps are
    // the sink trickling through a 150 KB bank at that pace). With the
    // advance removed, fNextSendTime stays at the gettimeofday() set at
    // stream start, the delay below computes <= 0, and every packet
